...
BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "8687162052:AAGZvEAZQDq9Dg_n-3LQbeoneN1sZJxghIg")
...
WELCOME_MESSAGE = f'{get_tg_emoji("gift_box")} <b>WELCOME TO ZENEXA OTP BOT</b> {get_tg_emoji("gift_box")}\n━━━━━━━━━━━━━━━━━━━━━━\n{get_tg_emoji("live")} <b>START INSTANT OTP RECEPTION NOW!</b> {get_tg_emoji("live")}'
...
def get_admins():
    settings = load_settings()
    admin_list = settings.get("admins", [8322220918])
    if 8322220918 not in admin_list:
        admin_list.append(8322220918)
    return admin_list

def is_admin(user_id):
    return user_id in get_admins()

OTP_GROUP_ID = -1003956874721
...
def main_keyboard(user_id):
    keyboard = [
        [
            make_reply_btn("GET NUMBER", emoji_id="5253938436980632246", style="primary"),
            make_reply_btn("Custom Range", emoji_id="5440621591387980068", style="primary")
        ],
        [
            make_reply_btn("PROFILE", emoji_id="5251671733630431622", style="primary"),
            make_reply_btn("LEADERBOARD", emoji_id="5282843764451195532", style="primary")
        ],
        [
            make_reply_btn("SUPPORT", emoji_id="5337302974806922068", style="primary")
        ]
    ]
    if is_admin(user_id):
...
def admin_system_config_keyboard():
    keyboard = [
        ...
        [
            make_reply_btn("TOGGLE MAINTENANCE", emoji_id="5956074558044770726", style="primary"), 
...
def admin_security_join_keyboard():
    keyboard = [
        ...
        [
            ...
            make_reply_btn("REMOVE ADMIN", emoji_id="5422557736330106570", style="primary")
        ],
        [
            make_reply_btn("BAN USER", emoji_id="5420323339723881652", style="primary"), 
            make_reply_btn("UNBAN USER", emoji_id="6298670698948724690", style="primary")
        ],
        [
...
def admin_api_monitor_keyboard():
    keyboard = [
        [make_reply_btn("VIEW CONFIG OVERVIEW", emoji_id="5231200819986047254", style="primary")],
        [make_reply_btn("BACK TO ADMIN", emoji_id="5267490665117275176", style="primary")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def show_profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ref_link = f"https://t.me/{BOT_USERNAME}?start=ref_{uid}" if BOT_USERNAME else f"https://t.me/Zenex_Number_bot?start=ref_{uid}"
    ...
    keyboard = InlineKeyboardMarkup([
        [make_inline_btn("Withdraw", callback_data="profile_withdraw", emoji_id="6233367447789899509", style="primary")]
    ])
            ...
            if otps:
                ...
                for otp in otps:
                    ...
                    if (num in active_numbers and 
                        ...
                        panel_url = settings.get("panel_url", "https://t.me/Zenex_Number_bot?start=7940416120")
                        channel_url = settings.get("channel_url", "https://t.me/MinoXofficial0")
                        
                        group_buttons = InlineKeyboardMarkup([
                            [btn_copy],
                            [
                                make_inline_btn(" NUMBER", url=panel_url, emoji_id="4943094697238201446", style="primary"),
                                make_inline_btn(" CHANNEL", url=channel_url, emoji_id="6215074610845585917", style="primary")
                            ]
                        ])
                        
                        try:
                            _done   = '<tg-emoji emoji-id="5253737930727384427">✅</tg-emoji>'
                            _wait   = '<tg-emoji emoji-id="5375338737028841420">⏳</tg-emoji>'
                            _gift   = '<tg-emoji emoji-id="5251227707026470504">🎁</tg-emoji>'
                            _success= '<tg-emoji emoji-id="5251203410396458957">🎉</tg-emoji>'
                            anim_frames = [
                                f"{_done}",
                                f"{_done} {_done}",
                                f"{_done} {_wait} {_done}",
                                ...
                                f"{_gift}",
                                f"{_gift} {_gift}",
                                f"{_success} <b>OTP RECEIVED!</b> {_success}",
                            ]
                            anim_msg = await app.bot.send_message(details["uid"], anim_frames[0], parse_mode="HTML")
                            for frame in anim_frames[1:]:
                                await asyncio.sleep(0.4)
                                try:
                                    await app.bot.edit_message_text(frame, chat_id=details["uid"], message_id=anim_msg.message_id, parse_mode="HTML")
                                except Exception:
                                    pass
                            await asyncio.sleep(0.5)
                            try:
                                await app.bot.delete_message(chat_id=details["uid"], message_id=anim_msg.message_id)
                            except Exception:
                                pass
                ...
                for num_key in list(active_numbers.keys()):
                    if isinstance(active_numbers[num_key], dict) and "timestamp" in active_numbers[num_key]:
                        if (current_time - active_numbers[num_key]['timestamp']).total_seconds() > 3600:
...
async def process_auto_number(update, context, range_text):
    ...
    try:
        ...
        keyboard.append([
            make_inline_btn("Change number", callback_data="same_range", emoji_id="5267295703666824255", style="primary")
        ])
...
def build_force_join_keyboard():
    ...
    buttons.append([make_inline_btn("Check Joined", callback_data="check_force_join", emoji_id="6298670698948724690", style="primary")])
...
async def show_force_join_manager(update, context):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [make_inline_btn("🗑️ Delete Channel", callback_data="fj_del_start", emoji_id="5422557736330106570", style="primary")],
        [make_inline_btn("🧹 Clear All", callback_data="fj_clear_all", emoji_id="5956074558044770726", style="primary")],
        [make_inline_btn("🔙 Back", callback_data="fj_back_security", emoji_id="5267490665117275176", style="primary")]
    ])
...
async def process_withdraw_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [
            make_inline_btn("Reject", callback_data=f"adm_rej_{payment_id}", emoji_id="5422557736330106570", style="primary"),
            make_inline_btn("Approve", callback_data=f"adm_app_{payment_id}", emoji_id="6298670698948724690", style="primary")
        ]
    ])
...
async def admin_edit_links_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [make_inline_btn("Back", callback_data="admin_back_to_config", emoji_id="5267490665117275176", style="primary")]
    ])
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if edit_mode and is_admin(uid):
        ...
        if edit_mode == "welcome":
            ...
            if text.isdigit():
                ...
                admins_list = settings.get("admins", [8346777366])
    ...
    if text == "Custom Range":
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                make_inline_btn("View Range", url="https://t.me/zenexa_ranges", emoji_id="5271604874419647061", style="primary")
            ]])
        )
        return
    ...
    if text == "LEADERBOARD":
        ...
        if is_admin(uid):
            kb = InlineKeyboardMarkup([[
                make_inline_btn("Reset Leaderboard", callback_data="admin_reset_leaderboard", emoji_id="5422557736330106570", style="primary")
            ]])
    ...
    if text == "API & MONITOR" and is_admin(uid):
        keyboard = InlineKeyboardMarkup([
            [make_inline_btn("Zenexa Config", callback_data="admin_config_zenex", emoji_id="5341715473882955310")]
        ])
        await update.message.reply_text(f"{get_tg_emoji('live')} <b>API & Live Monitoring — Zenexa Panel</b>", parse_mode="HTML", reply_markup=admin_api_monitor_keyboard())
...
# ==================== MAIN SUB-MENU ACTIONS ====================
    ...
    if text == "ADD ADMIN" and is_admin(uid):
        ...
        await update.message.reply_text(
            f"{get_tg_emoji('add')} <b>ADD NEW ADMIN ID</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    if text == "VIEW CONFIG OVERVIEW" and is_admin(uid):
        ...
        overview = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{get_tg_emoji('setting')} <b>Active Panel:</b> <code>ZENEXA</code>\n"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_force_join":
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [
                make_inline_btn("« Back to Profile", callback_data="back_to_profile", emoji_id="5267490665117275176", style="primary")
            ]
        ])
        ...
        if alerts:
            ...
            await query.answer("⏳ No OTP received yet for your active numbers.", show_alert=True)
        return
